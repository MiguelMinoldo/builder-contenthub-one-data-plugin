...
const metaFields = ["version", "status", "type", "createdBy", "createdAt", "updatedBy", "updatedAt", "publishedBy", "publishedAt"];
const chFieldTypes = ["Text", "Boolean", "Number", "Symbol", "Reference", "Media", "ShortText", "LongText" ]
    ...
    return {
      async getResourceTypes() {
        ...
        return contentTypesCH1.map((type) => {
          const acceptableFields = type.fields.filter((field) => chFieldTypes.includes(field.type));
      ...
      async getEntriesByResourceType(id: any, options: { searchText: string; resourceEntryId: string; }) {
